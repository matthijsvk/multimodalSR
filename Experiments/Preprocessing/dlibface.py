...
for f in sys.argv[1:]:
	...
	if (top > add_height): top -= add_height
	else: top = 0
	if(bot + add_height < img_height):bot += add_height
	else: bot = img_height

	if (left > add_width): left -= add_width
	else: left = 0
	if (right + add_width < img_width):	right += add_width
	else: right = img_width
