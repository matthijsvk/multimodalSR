...
def main():
    ...
    try:
        prevResults = unpickle(resultsPath)
    except:
        prevResults = {}
    import pdb;
    pdb.set_trace()
...
def trainManyNetworks(networks):
    ...
    for network in tqdm(networks, total=len(networks)):
        ...
        pprint(vars(network))
        try:
            model_save, test_results = trainNetwork(AUDIO_LSTM_HIDDEN_LIST=network.AUDIO_LSTM_HIDDEN_LIST,
            ...
            name = network.runType + "_" + '_'.join([str(layer) for layer in network.AUDIO_LSTM_HIDDEN_LIST]) \
                   + network.cnn_features + ('_'.join([str(layer) for layer in
                                                       network.LIP_RNN_HIDDEN_LIST]) if network.LIP_RNN_HIDDEN_LIST != None else "") \
...
def trainNetwork(AUDIO_LSTM_HIDDEN_LIST, CNN_NETWORK, cnn_features, lipRNN_bidirectional, LIP_RNN_HIDDEN_LIST,
                 DENSE_HIDDEN_LIST,
    ...
    best_val_acc, test_acc, test_topk_acc = network.loadPreviousResults(model_save)
...
def getModelPaths(AUDIO_LSTM_HIDDEN_LIST, CNN_NETWORK, DENSE_HIDDEN_LIST, LIP_RNN_HIDDEN_LIST, allowSubnetTraining,
    ...
    root_dir = os.path.expanduser('~/TCDTIMIT/combinedSR/' + dataset)
    store_dir = root_dir + os.sep + "results" + os.sep + (
    "CNN_LSTM" if LIP_RNN_HIDDEN_LIST != None else "CNN") + os.sep + datasetType
