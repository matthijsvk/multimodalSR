...
def train(train_fn, val_fn,
    ...
    def evalTRAINING(trainingSpeakerFiles, LR, shuffleEnabled):
        train_cost = 0;
        val_err = 0;
        val_cost = 0;
        nb_train_batches = 0;
        ...
        for speakerFile in tqdm(trainingSpeakerFiles, total=len(trainingSpeakerFiles)):
            ...
            logger_train.info("\ttraining cost:     %s", train_cost_one / train_batches_one)
            logger_train.info("\tvalidation cost:   %s", val_cost_one / val_batches_one)
    ...
    def evalTEST(testSpeakerFiles):
        ...
        for speakerFile in tqdm(testSpeakerFiles, total=len(testSpeakerFiles)):
            ...
            logger_train.debug("  this speaker results: ")
            logger_train.info("\ttest cost:   %s", test_cost_one / test_batches_one)
            logger_train.info("\vtest error rate:  %s %%", test_err_one / test_batches_one)
