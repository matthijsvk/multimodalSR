...
from utils import helpFunctions


###################################################################
### Before running this, make sure all files are in lowecase!!  ###
# to do this in Linux: run `find . -depth -print -execdir rename -f 'y/A-Z/a-z/' '{}' \;` in the TIMIT root directory.
###################################################################
...
def prepareWAV_HTK(baseDir=os.path.expanduser('~/TCDTIMIT/TIMIT/TIMIT'),
    ...
    wavFilePath = fileStoreDir + helpFunctions.os.sep + 'wavPaths.txt'
    helpFunctions.writeToTxt(wavs, wavFilePath)

    wavAndMFCCFilePath = fileStoreDir + helpFunctions.os.sep + 'wavAndMFCCPaths.scp'
    helpFunctions.writeToTxt(wavAndMFCC, wavAndMFCCFilePath)
    ...
    wavConfigPath = fileStoreDir + helpFunctions.os.sep + 'wav_config'
    helpFunctions.writeToTxt(wav_config, wavConfigPath)
