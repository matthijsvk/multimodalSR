...
# Vocab size for proteins
vocab_size = 20

# Categories for proteins
num_categories = 3
...
def generate_batch(batch_size=50, seq_length=20):
    X = np.zeros((batch_size, seq_length, vocab_size), dtype='int32')
    y = np.zeros((batch_size, seq_length, num_categories), dtype='int32')
    z = np.zeros((batch_size, seq_length), dtype='int32')
    a = np.zeros(batch_size, dtype='int32')

    for i in range(batch_size):
        for j in range(seq_length):
            Xval = random.randint(0, 19)

            ## This is the very simple function that we hope the LSTM model will learn:
            ## i.e. characters in the interval [0, 6] result in category 0
            ## characters in the interval [7, 12] result in category 1
            ## characters in the interval [13, 19] result in category 2
            if Xval <= 6:
                yval = 0
            elif 5 < Xval <= 12:
                yval = 1
...
def main(num_epochs=NUM_EPOCHS):
    ...
    # The output of this stage is (batch_size, num_categories)
    l_out = lasagne.layers.DenseLayer(l_forward_2, num_units=num_categories, W=lasagne.init.Normal(),
                                      nonlinearity=lasagne.nonlinearities.softmax)

    # Theano tensor for the targets
    target_values = T.ivector('target_output')
    # target_values = T.imatrix('target_ouput')
    ...
    #     sample_ix = []
    #
    #     x, _ = generate_batch(len(generation_phrase) - SEQ_LENGTH, 1, generation_phrase, 0)
    #
    #     for i in range(N):
